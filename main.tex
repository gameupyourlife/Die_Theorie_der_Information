...
wobei jeweils $R \in \mathbb{R}^{N \times M}$ das \textit{Response}, $s \in \mathbb{R}^M$ die Signale und $n \in \mathbb{R}^N$ das Rauschen beschreiben. Jedes Element von $d$ entspricht einem Linienintegral aus \cref{k4.2.tomo.einl.linig}.
...
\end{equation}
 ...
 Die Liste $A$ besteht aus allen Schnittpunkten $[S_{x,i} \ | \ i \in \{0\} \cup \mathbb{N}_{< (\llcorner x_{end} - x_{start} \lrcorner)} ]$. Die Listen $B$ und $C$ berechnet man analog. 
...
Hat man alle sortierten Schnittpunkte, so kann man auch die Gewichtungen, i.e. die Abstände zwischen den jeweiligen Punkten, einfach berechnen. 
